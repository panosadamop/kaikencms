...
import ltd.dnacreative.cms.config.KafkaProperties;
...
class KaikenCmsKafkaResourceIT {
    ...
    void setup() {
        ...
        KaikenCmsKafkaResource kafkaResource = new KaikenCmsKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/kaiken-cms-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kaiken-cms-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
